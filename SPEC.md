...
| 2 | `strata_md` | Semantics declared in a human-authored `strata.md` intent file. Treated as high-confidence human input, below `user_defined` (which requires an explicit correction record) but above automated inference sources. |
| 3 | `ddl_comment` | Extracted from a `COMMENT ON COLUMN` or `COMMENT ON TABLE` statement in the database DDL. Represents explicit human intent stored in the database. |
| 4 | `schema_constraint` | Derived from schema metadata: primary key constraints, foreign key constraints, NOT NULL constraints, check constraints, unique constraints. |
| 5 | `catalog_import` | Imported from an external metadata catalog (DataHub, OpenMetadata, Atlan). Trust depends on the catalog's own data quality; treat as high-confidence human-authored metadata unless the catalog's provenance indicates otherwise. |
| 6 | `code_extracted` | Extracted from transformation or application code: dbt model definitions, SQLAlchemy ORM models, migration files, application-layer docstrings. |
| 7 | `log_inferred` | Inferred from query log analysis (e.g. `pg_stat_statements`). Represents observed usage, not intended semantics. |
| 8 | `llm_inferred` | Generated by an LLM prompt. Lowest trust among automated sources. |
...
**Scope:** document  
**Description:** Every `provenance.source_type` MUST be one of the eight defined values: `user_defined`, `strata_md`, `ddl_comment`, `schema_constraint`, `catalog_import`, `code_extracted`, `log_inferred`, `llm_inferred`.

**Passes:**
```yaml
provenance:
  source_type: strata_md
  confidence: 0.95
  human_reviewed: false
```
